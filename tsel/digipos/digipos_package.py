...
if __name__ == '__main__':
    ...
    conf.setAppName("Digipos package")
    ...
    # query
    path_query_digipos_package = args.query + "/digipos_package.sql"
    # output
    ...
    createScheme(path=path_src,
                 delimiter="~",
                 schemaString="ACTIVATION_ID STATUS A_NUMBER B_NUMBER PRICE VAS_ID PRODUCT_ID_DESC PRODUCT_NAME BRAND TRX_TYPE OUTLET_ID CHANNEL_ID CREATED_AT CREATED_BY UPDATED_AT UPDATED_BY EXT_TRX_ID STATUS_DESC CASHBACK_STATUS CASHBACK_DESC SERIAL_NUMBER ZONE LOS",
                 name = "digipos_package_src")

    # digipos_package before
    createScheme(path=path_current_hive_before,
                 delimiter="~",
                 schemaString="ACTIVATION_ID STATUS A_NUMBER B_NUMBER PRICE VAS_ID PRODUCT_ID_DESC PRODUCT_NAME BRAND TRX_TYPE OUTLET_ID CHANNEL_ID CREATED_AT CREATED_BY UPDATED_AT UPDATED_BY EXT_TRX_ID STATUS_DESC CASHBACK_STATUS CASHBACK_DESC SERIAL_NUMBER ZONE LOS",
                 name="digipos_package_before")

    # digipos_package after
    createScheme(path=path_current_hive_after,
                 delimiter="~",
                 schemaString="ACTIVATION_ID STATUS A_NUMBER B_NUMBER PRICE VAS_ID PRODUCT_ID_DESC PRODUCT_NAME BRAND TRX_TYPE OUTLET_ID CHANNEL_ID CREATED_AT CREATED_BY UPDATED_AT UPDATED_BY EXT_TRX_ID STATUS_DESC CASHBACK_STATUS CASHBACK_DESC SERIAL_NUMBER ZONE LOS",
                 name="digipos_package_after")
    ...
    ########## 4. Output ##########
    sqlContext.sql("select * from digipos_package").map(toCSVLine).saveAsTextFile(path_output)
