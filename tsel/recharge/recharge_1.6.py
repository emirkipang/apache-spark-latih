...
#         data = myfile.read()
#     sqlContext.sql(data).coalesce(12).registerTempTable(str.replace(basename(path),".sql",""))
#
# def toCSVLine(data):
#     return '|'.join(str(d) for d in data)
#
# def createScheme(path, delimiter, schemaString, name):
#     lines = sc.textFile(path).coalesce(12)
#     parts = lines.map(lambda l: l.split(delimiter)).coalesce(12)
#     data = parts.map(lambda p: tuple(p)).coalesce(12)
#     fields = [StructField(field_name, StringType(), True) for field_name in schemaString.split()]
#     schema = StructType(fields)
#     sqlContext.createDataFrame(data, schema).coalesce(12).registerTempTable(name)
...
if __name__ == '__main__':
    ...
    # src
    path_src_mkios = "file://" + base_query + "/mkios/prc/*"
    path_src_urp = "file://" + base_query + "/urp/prc/*"
    ...
    path_ref_ad = "file://" + base_query + "/ref/adreference/list_ad.csv"

    # query
    path_query_lacima = base_query + "/query/lacima.sql"
    path_query_ad_reference = base_query + "/query/ad_reference.sql"
    path_query_recharge = base_query + "/query/recharge.sql"
    path_query_output = base_query + "/query/output.sql"

    # output
    path_output = "file://" + base_query + "/out/recharge_summary_temp"
    ...
    # ad ref
    createScheme(path=path_ref_ad,
                 delimiter="|",
                 schemaString="no regional branch subbranch cluster leadadname leadadmsisdn leadadcode",
                 name="list_ad")
    ...
    # Create adref
    applyQuery(path_query_ad_reference)
    ...
    sqlContext.sql("select * from output").map(toCSVLine).coalesce(10).saveAsTextFile(path_output)
